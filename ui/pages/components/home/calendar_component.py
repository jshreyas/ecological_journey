...
def calendar_container(grouped_videos_by_day):
        ...
        with calendar_grid:
            ...
            for day in range(1, days_in_month + 1):
                ...
                with ui.card().classes(
                    "h-20 p-2 bg-gray-50 items-center border border-gray-300 shadow-sm hover:shadow-md transition rounded-lg"
                ):
                    ...
                    if videos:
                        ...
                        with ui.row().classes("gap-1 items-center"):
                            for playlist, color_class in playlist_set:
                                ...
                                ui.element("div").classes(f"{color_class} {border_class} w-3 h-3 rounded-full").tooltip(
                                    playlist
                                )
                    # if videos:
    ...
    with ui.column().classes("w-full h-full"):
        ...
        with ui.row().classes("w-full justify-between items-center"):
            ui.button(icon="arrow_back", on_click=lambda: change_month(-1)).props("flat").classes("hover:bg-blue-100")
            month_label = ui.label("").classes("text-xl font-bold")  # Dynamically updated label
            ui.button(icon="arrow_forward", on_click=lambda: change_month(1)).props("flat").classes("hover:bg-blue-100")
        ...
        with ui.row().classes("w-full h-full max-w-5xl flex-1 bg-white rounded-lg shadow-lg overflow-hidden"):
            with ui.column().classes("w-full h-full overflow-y-auto"):
                calendar_grid = ui.grid(columns=7).classes("gap-2 w-full p-2")
                render_calendar()
