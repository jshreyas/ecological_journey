...
class TeamTab:
    """Component for displaying team tab"""
    ...
    def create_tab(self, container):
        """Create the team tab UI"""
        self.container = container
        self.refresh()

    def refresh(self):
        """Refresh the team tab with current video data"""
        if not self.container:
            return

        self.container.clear()
        with self.container:
            self._create_team_ui()

    def _create_team_ui(self):
        """Create the team UI"""

        if not self.home_state.user:
            both = fetch_teams_for_user_jd(44)
        else:
            both = fetch_teams_for_user(self.home_state.user.id)
        ...
        with ui.column().classes("w-full p-4 border border-gray-300 rounded-lg bg-white shadow-md gap-3"):
            with ui.row().classes("w-full justify-between items-center"):
                ...
                ui.button(
                    on_click=(caught_john_doe if not self.home_state.user else create_new_team),
                    icon="save",
